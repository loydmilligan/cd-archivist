...
- [x] {agent: lane-2, depends: shell-route, id: landing-grid} Render the landing card grid at `/library` (no panel selected) inside `archivist/service/library_landing.py` (new module). Four v1 cards (downloads, inbox, disk, library) each with eyebrow + title + sub + a small preview shape per build-prompt §2 landing-state: per-track pip strip for Downloads, album rows with READY pip for Inbox, three horizontal usage bars for Disk, recent-imports list for Library. Three v2 ghost cards beneath, 42% opacity, non-clickable. Card clicks navigate to `/library/{panelId}`.
...
- New `archivist/service/library_switcher.py` with `render_switcher(*, active, rip_telemetry, library_telemetry) -> str` and a `SWITCHER_JS` IIFE constant. Switcher recipe: brand mark + `·` separator + active-label button with `▾` caret + popover with `Surfaces` header + two rows (rip / library) each with `✓` check (active-row only via `.is-on`) + telemetry meta. Popover starts collapsed (`hidden` attribute); button toggles via `data-switcher-toggle`. Click-outside + Escape close it.
- Wired into `kanban_page._render_header_bar`: signature gained `active_surface`, `rip_telemetry`, `library_telemetry` kwargs. Legacy bare `<span class="cda-brand-mark cda-brand-mark--header">` removed — the switcher carries the brand mark. The legacy `<span class="wordmark">cd-archivist</span>` is also gone (redundant alongside the brand lockup). `render_kanban_page` derives `rip_telemetry` from live state (`N ripping · M review` from active_rip + stats["review"]) and passes a static `library_telemetry = "3 inbox · 1 download"` placeholder per build-prompt §2.
- `library_page.render_library_page` calls the same `_render_header_bar` with `active_surface="library"`. Both pages inject `<script>{SWITCHER_JS}</script>` for the dropdown toggle.
- CSS: `library.css` carries the `.cda-switcher-*` rules; it's now loaded on `/rip` too (after `cda.css`) so the switcher renders correctly on both surfaces. Updated `test_kanban_page::test_kanban_page_class_names_resolve_against_static_css` haystack to include library.css.
- Voice-pass: added `✓` to `_ALLOWED_GLYPHS` (functional Unicode glyph; build-prompt §2 explicitly calls for a "moss check" on the active surface row).
- New tests: `test_breadcrumb_switcher.py` (11 cases) — render_switcher unit tests (brand mark + separator, button label = active, exactly-one is-on, hrefs, telemetry, collapsed-by-default, ValueError on bad active) + integration (switcher on /rip with rip active, switcher on /library with library active, JS injection on both, no-duplicate brand mark).

### 2026-05-19 — orc (lane-2) — sidebar-panels + landing-grid + panel-placeholder landed (24 new tests, 659/659 green)

- New shared inventory `archivist/service/library_panel_inventory.py` with `PRIMARY`, `GHOSTS`, `BY_ID` — ported verbatim from the JSX reference `LIB_PANELS` so per-panel copy/glyph/sub lives in one place across all three lane-2 modules.
- New `archivist/service/library_sidebar.py` with `render_sidebar(*, active_panel_id=None) -> str`. Four groups (QUEUES/HEALTH/BROWSE/COMING · v2), four v1 anchors to `/library/{id}`, three v2 ghost `<div>`s with `aria-disabled="true"` + `soon` chip + `is-ghost` class. Count badges only render when `spec["count"]` is set (Downloads=1, Inbox=3).
- New `archivist/service/library_landing.py` with `render_landing_grid()`. Header copy ("Library manager · cm4 music stack" / "The post-rip side of the rig"); 4 v1 anchor cards with build-prompt-named preview shapes (`--pip-strip`, `--inbox-rows`, `--usage-bars`, `--recent-list`); 3 v2 ghost `<article>`s with `soon · v2` chip. Disk preview's `/mnt/archive` at 78% carries `is-amber` per build-prompt §3 thresholds.
- New `archivist/service/library_panels.py` with `render_panel(panel_id)`. Per-panel header (eyebrow / h1 / sub from inventory) + 3 ghost rows + `in design` tag with foot copy.
- Lane-1's shell stubs in `library_page.py` are now superseded — the lazy imports succeed and the stub fallbacks never fire. Stubs left in place as defensive belt-and-suspenders (cheap, and if a module is removed in a future refactor the shell still renders).
- New tests: `test_library_sidebar.py` (8 cases), `test_library_landing.py` (7 cases), `test_library_panels.py` (9 cases including the 4-parametrize). 24 total. Full suite: 659/659.
