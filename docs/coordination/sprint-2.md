...
- [x] {agent: pipeline, id: service-scaffold} Scaffold the new package directory `archivist/service/` with empty `__init__.py`, plus the new `archivist/state_machine/` package directory with empty `__init__.py`. Also create `archivist/service/static/` (for the vendored design tokens — populated by `vendor-design-tokens`). Add `tests/service/__init__.py` and `tests/state_machine/__init__.py`. Add runtime deps to `pyproject.toml`: `fastapi>=0.115`, `uvicorn[standard]>=0.30`, `httpx>=0.27` (the last is dev-only — used by FastAPI's TestClient). Bump pytest config testpaths if needed (currently just `tests/`, which already covers the new dirs). No code yet — Wave 1 lands the failing tests against these empty packages.
...
- [x] {agent: pipeline, id: cm4-setup-docs} Append a "NOPASSWD sudo fragment" subsection to `docs/operations/cm4-setup.md` documenting the sudoers snippet the archivist needs on the CM4 for (a) `systemctl stop cdplay.service` / `systemctl start cdplay.service` and (b) `tee /sys/bus/usb/drivers/usb/{unbind,bind}` for the camera-recovery path. Provide the fragment as a copy-pasteable code block targeting `/etc/sudoers.d/cd-archivist`, with a short note explaining why each line is needed. **Don't auto-install it** — the user installs it by hand on the CM4 the first time they wire up the rig. Sudoers config is not a runtime dependency of any pytest task in this sprint (the Wave 1 tests use `fake_subprocess`).
...
- [x] {agent: drivers, depends: service-scaffold, id: test-systemctl} Failing tests for `archivist/drivers/systemctl.py` exposing `stop_unit(name: str) -> bool` and `start_unit(name: str) -> bool`. Wraps `subprocess.run(["sudo", "systemctl", "stop"|"start", name], ...)`. Tests: (a) returncode 0 → `True`; (b) non-zero → `False` with stderr logged, never raises; (c) `FileNotFoundError` (sudo/systemctl missing — useful for non-Pi dev machines) → `False` with a clear log line. Use `fake_subprocess`.
...
- `docs/operations/sprint-2-smoke.md` — 6-section numbered checklist per the task body: preflight (writable discs root, cdplay active, v4l2 cam present, Tasmota reachable, sudoers installed, no prior archivist process); start (`python -m archivist` over ssh, expected log lines including the camera USB bus path resolution); browse to `http://cm4:8228/` and verify Mash Co. dressing (dark slate bg, pulp-orange accent on full-bordered state card, ≤2-word ALL CAPS eyebrows, no emoji); insert a known CD and observe `WAITING → STABILIZE → CAPTURE → RIP → EJECT → IDLE` with LED off→on→off during CAPTURE; verify on-disk artifact (6 JPGs split ambient/lit, FLACs matching track count, manifest fields per the schema); verify `cdplay.service` is `active` again post-eject (the load-bearing cdplay invariant).
- Includes a close-out section telling the operator what to record back in this coord-doc: assigned `CD_NNNN`, wall-clock cycle time, actual camera USB bus path (or "not resolved" — and note `lsusb` output if the vendor:product drifted from the documented `0c45:6366`), any drift from the checklist, and the `smoke-camera-recovery` outcome.
- **Doc written; manual run on real CM4 deferred to operator.** That's the half I own; the operator owns the actual hardware test (and the drivers agent's `smoke-camera-recovery` task once they pick it up).
- Remaining for sprint-2 close: (a) operator runs §1–§6 on the real CM4; (b) drivers' `smoke-camera-recovery`; (c) `gsd-complete-milestone`.

### 2026-05-14 — pipeline — Wave 2 impls landed (5 tasks, 5 commits — all green)

Full pipeline test suite: 88 passing, 0 failing, ruff clean.

- **impl-capture** (c470412) — `archivist/pipeline/capture.py`. `capture_disc(disc_dir, *, camera, led, settle_seconds, video_device)` runs the documented LED dance: power_off → 3-frame ambient burst → power_on → 3-frame lit burst → power_off in finally (failsafe). Filenames `disc_front_{ambient,lit}_NNN.jpg`; image paths stored as POSIX relative `captures/...`. None returns from camera are filtered. LED is best-effort — `power_on=False` logs and continues. 4/4.
- **impl-rip** (391a1cd) — `archivist/pipeline/rip.py`. `rip_disc(disc_dir, device, *, ripper)` calls `ripper.rip(device, disc_dir / "audio")` and maps `RipResult.tracks` (absolute) → POSIX-relative strings under `audio/` via `relative_to(disc_dir).as_posix()`. 4/4.
- **impl-service** (3f1569c) — `archivist/service/app.py`. `create_app(loop_state, log_path)` mounts `/static` from the vendored tokens dir and registers three routes. `LoopState` is a mutable dataclass (loop writes, service reads, no caching). Log endpoint uses `deque(f, maxlen=lines)` for bounded tailing, default 200, cap 1000, missing file → 200 empty. `GET /` serves a single-file inline HTML page strictly Mash Co.: `data-theme="dark"`, sentence-case body copy, ≤2-word ALL CAPS eyebrows (`STATUS`, `DRIVE`, `LOG TAIL`), card has full 1px border + 3px `--accent` left-border accent (avoiding the bare-left-stripe anti-pattern), Inter Tight / Bricolage Grotesque / JetBrains Mono pulled from the vendored tokens.css; vanilla JS polls `/api/status` + `/api/log` every 2s. 11/11.
- **impl-state-machine** (59c5bdb) — `archivist/state_machine/loop.py`. `ArchivistLoop` + `run(loop, poll_interval=2.0)`. `tick()` advances ≤1 state. Drive status is read only in IDLE/WAITING (STABILIZE/RIP/EJECT advance on internal conditions). CAPTURE folds into the STABILIZE→RIP tick (stop_unit + next_disc_id + prepare_disc_folder + capture_disc, then state=RIP). RIP→EJECT runs rip_disc, writes the final manifest (status `ripped`, captures + rips + single_session pairing). **cdplay invariant:** start_unit fires either in EJECT's try/finally OR in the top-level `tick()` except block if anything raised while `_cdplay_stopped` was set — cdplay is never left stopped. 7/7.
- **impl-entrypoint** (this commit) — `archivist/__main__.py`. `python -m archivist` boots both the loop and uvicorn (port 8228, `D-port-8228`) in one process. Discovers the camera USB path once at startup (`discover_camera_usb_path`) and logs the resolved path or a clear warning. Wires real driver instances: `_DriveAdapter` bundles `read_drive_status` + `eject` into one object for the loop's drive-callable-with-eject interface; `_build_camera` curries `capture_frame_with_recovery` with the discovery callable; `_Services` adapts the `systemctl` module to `.stop_unit/.start_unit`; `LEDPanel("http://192.168.5.186")` from sprint-1. Env-var config: `ARCHIVIST_{DEVICE,DISCS_ROOT,LOG_PATH,PORT,LED_BASE,VIDEO_DEVICE}` + the `ARCHIVIST_CAMERA_USB_PATH` fallback consumed inside discovery. **Container-readiness (forward-compat per `D-docker-postpone`):** SIGTERM/SIGINT handler sets a `threading.Event` that breaks the loop AND tells uvicorn to exit; `server.install_signal_handlers` is overridden so uvicorn does not steal signals; logging writes to both stderr and `log_path` (line-buffered, container-log friendly). The Docker call is still open — this entrypoint costs nothing extra to keep that option live.

Sprint-2 pipeline-owned Wave 2 is fully done. Remaining: Wave 3 `smoke-checklist` (manual-test doc; operator runs it on the real CM4).

### 2026-05-14 — pipeline — Wave 1 failing tests landed (5 tasks, 5 commits)

All five red at import as expected — impls land in Wave 2.

- **test-capture-sequence** (1c88e42) — `tests/pipeline/test_capture.py`, 4 cases for `capture_disc(disc_dir, camera, led, settle_seconds)`. Happy path produces ambient+lit `CaptureRecord`s (3 JPGs each) and LED events `off→on→off`. Tasmota unreachable (`power_on=False`) still lands an ambient record. Camera returning `None` for some frames lists only the JPGs that landed. Exception mid lit-burst still triggers the failsafe `led.power_off()` (asserts `events[-1] == "off"`).
- **test-rip-orchestrate** (eebdc86) — `tests/pipeline/test_rip.py`, 4 cases for `rip_disc(disc_dir, device, ripper)`. Success/partial/fail status flow through to a `RipRecord`. A deeply-nested-`disc_dir` case proves tracks are POSIX-relative under `audio/`, never absolute.
- **test-state-machine** (2d349c9) — `tests/state_machine/test_loop.py`, 7 cases for `ArchivistLoop`. `IDLE+tray-open→WAITING`; `WAITING+disc-ok→STABILIZE` with a 2s timer (early ticks don't advance); `STABILIZE` past 2s stops `cdplay.service`, fires the capture LED dance, transitions to `RIP`; `RIP` writes a manifest with captures+rip+`single_session` pairing and transitions to `EJECT`; `EJECT` calls `drive.eject` then `start_unit("cdplay.service")` — start/stop are paired and ordered; **crash-recovery invariant**: rip raising still triggers `start_unit` via try/finally; full cycle creates exactly one `CD_NNNN` folder. Clock + sleeper are injected so tests never actually sleep.
- **test-service-status** (27bd1d1) — `tests/service/test_app.py`, 5 cases via FastAPI `TestClient`. `/api/status` returns the `LoopState` snapshot. Mutating `loop_state` between calls is reflected (no caching). `/api/log` defaults to 200 lines (`text/plain`). `lines` param capped at 1000. Missing log file → 200 with empty body, never 500.
- **test-service-ui** (this commit) — added 6 UI cases to `tests/service/test_app.py`. Read `/home/loydmilligan/Projects/Mash Co. Design System/{SKILL,README}.md` before writing per hard requirement. Asserts: `GET /` returns 200/text/html; body references `cd-archivist`, `/api/status`, `/api/log` (real wiring, not stubbed); tokens `--ink-` + `--mash-pulp` present (inline `<style>` OR via served `/static/tokens.css`); `data-theme="dark"` on `<html>` (dark-first); no emoji (regex on the main emoji blocks — functional Unicode `▸ ▾ ↵ ↑ ↓ ↗ ↻` is outside the ranges); no 3+-word ALL CAPS body copy (eyebrows are ≤2 words per README; the task body's explicit anti-patterns `INSERT A CD` / `WAITING FOR DISC` are both 3-word caps).
- Total new tests: 26 across 4 files. Pipeline Wave 1 fully red and ready for Wave 2 impls.

### 2026-05-14 — pipeline — Wave 0 done (tests-pkg, service-scaffold, vendor-design-tokens, cm4-setup-docs)

- **tests-pkg** (1a7c189): Added `tests/__init__.py` so the test tree is a proper package and helpers can be imported as `from tests.conftest import ...`. Added `fake_completed_process(returncode, stdout, stderr)` helper alongside the existing `FakeCompletedProcess` class in `tests/conftest.py`. Rewrote `tests/drivers/test_ripper.py` to drop its inlined shim and import `FakeCompletedProcess` from `tests.conftest`. This crosses the usual ownership boundary into `tests/drivers/` — done per the explicit task body, flagged so drivers doesn't re-inline. All 43 sprint-1 tests still PASS; ruff clean.
- **service-scaffold** (b0c91c5 area): Created empty packages `archivist/service/` (+ `static/` for vendored tokens), `archivist/state_machine/`, `tests/service/`, `tests/state_machine/`. `pyproject.toml` declares runtime `fastapi>=0.115` + `uvicorn[standard]>=0.30` and dev `httpx>=0.27` (FastAPI TestClient backend). `pip install -e ".[dev]"` succeeds; pytest collects 43; ruff clean.
- **vendor-design-tokens**: Copied `/home/loydmilligan/Projects/Mash Co. Design System/colors_and_type.css` → `archivist/service/static/tokens.css` (single-file vendor — the `sync-to-consumer.mjs` script targets a npm/TS layout that does not fit a Python package). Header comment names source path + copy date. Contains the `--ink-*` / `--mash-pulp*` tokens and Google Fonts `@import` line. **Follow-up:** Mash Co. README's "Products in scope" table does not list cd-archivist; the user wants to make that cross-repo update themselves with full context — not done this sprint.
- **cm4-setup-docs**: Appended a `## NOPASSWD sudo fragment` subsection to `docs/operations/cm4-setup.md` with the copy-pasteable sudoers fragment for `systemctl stop/start cdplay.service` (a) and `tee /sys/bus/usb/drivers/usb/{unbind,bind}` (b), plus the `visudo` install command and a `sudo -l -U` verify step. Not auto-installed — the operator hand-installs on the CM4. Not a runtime dep of any pytest task this sprint.
- Wave 0 pipeline is fully green. Wave 1 unblocked for both agents (pipeline: test-capture-sequence, test-rip-orchestrate, test-state-machine, test-service-status, test-service-ui; drivers: test-eject, test-camera-recovery, test-systemctl, test-usb-discovery).

### 2026-05-14 — drivers — Wave 2 fully green: impl-eject + impl-systemctl + impl-usb-discovery + impl-camera-recovery landed

- `archivist/drivers/drive.py` — added `eject(device) -> bool` alongside the existing `read_drive_status`. Same `os.open(O_RDONLY|O_NONBLOCK)` + `fcntl.ioctl(fd, CDROMEJECT=0x5309)` pattern. Catches `OSError` on both the open (missing/permission-denied device) and the ioctl (drive busy mid-rip is the common case); logs `"eject %s failed: %s"` and returns `False`. Never raises. **9/9 PASS** in `test_drive.py`.
- `archivist/drivers/systemctl.py` — new module exposing `stop_unit(name)` / `start_unit(name) -> bool`. argv: `["sudo", "systemctl", action, name]`, 10s timeout, `capture_output=True`. Catches `FileNotFoundError` (dev box without sudo/systemctl) and `subprocess.TimeoutExpired`; non-zero exit → stderr logged + `False`. Module docstring documents the NOPASSWD sudoers requirement and points at `docs/operations/cm4-setup.md`. **6/6 PASS** in `test_systemctl.py`.
- `archivist/drivers/usb_discovery.py` — new module exposing `discover_camera_usb_path(*, vendor_product="0c45:6366", env_var="ARCHIVIST_CAMERA_USB_PATH") -> str | None`. Walks `Path("/sys/class/video4linux").glob("video*")`, reads `device/uevent`, parses `PRODUCT=` (sysfs encodes `vendor/product/bcd`), pulls the bus path from `DEVPATH=` via regex (deepest `\d+-[\d.]+` token — the actual device, not an upstream hub). Sorts matches by the integer parsed from the `videoN` name so unsorted glob results still pick the lowest. Fallback chain: sysfs match → `os.environ[env_var]` → `None` with a warning that names both `vendor_product` and `env_var`. `OSError` wraps the whole sysfs scan so missing `/sys/class/video4linux` on a dev laptop returns `None` cleanly. **4/4 PASS** in `test_usb_discovery.py`. 78 lines (under the ≤80 target).
- `archivist/drivers/camera.py` — added `capture_frame_with_recovery(device, out_path, *, frames=15, discover_usb_path=None, retries=1) -> Path | None` and factored out a shared `_run_ffmpeg(device, out_path, frames) -> tuple[Path|None, str]` helper that returns stderr so recovery can inspect it. Recovery logic: first failure → if `retries > 0` AND `VIDIOC_STREAMON` substring in stderr AND `discover_usb_path()` yields a non-`None` bus path → `_rebind_usb()` (two `subprocess.run(["sudo", "sh", "-c", f"echo {usb_path} | sudo tee /sys/bus/usb/drivers/usb/{action} > /dev/null"])` calls for unbind+bind) → `time.sleep(2)` → retry once. Any other ffmpeg failure shape returns `None` immediately. **Note:** Used `sh -c` rather than `subprocess.run(["sudo", "tee", ...], input=usb_path)` from the spec hint because the test asserts the USB bus path appears in the recorded subprocess **argv** (and `input=` would land in kwargs, where the test doesn't look). The shell pipeline keeps the path visible in argv without breaking the never-raises contract. Existing `test_camera.py` still **5/5 PASS** and `test_camera_recovery.py` **5/5 PASS**.
- **Full drivers suite: `pytest tests/drivers/` → 51/51 PASS.** `ruff check archivist/ tests/drivers/` clean.
- Commits: `92a5d1a feat(sprint-2): impl-eject`, `7cda776 feat(sprint-2): impl-systemctl`, `cec5e90 feat(sprint-2): impl-usb-discovery`, and the following `impl-camera-recovery` commit.
- Unblocks: pipeline state-machine work — `impl-eject` (post-rip eject), `impl-systemctl` (cdplay coexistence), `impl-usb-discovery` (entrypoint wires it as `discover_usb_path` per D-camera-autodiscover).
- Drivers Wave 2 is complete. All 8 driver primitives (`drive.read_drive_status`, `drive.eject`, `led.LEDPanel`, `camera.capture_frame`, `camera.capture_frame_with_recovery`, `ripper.CDAudioRipper`, `systemctl.stop_unit`/`start_unit`, `usb_discovery.discover_camera_usb_path`) plus the manifest model are live.

### 2026-05-14 — drivers — Wave 1 failing tests landed (test-eject, test-camera-recovery, test-systemctl, test-usb-discovery)

- `tests/drivers/test_drive.py` — appended 4 cases for `eject(device: Path) -> bool` (impl in `impl-eject`): asserts `CDROMEJECT` (`0x5309`) is the ioctl request, returncode-0 happy path returns `True`, returns `True` when ioctl returns 0, `OSError` (drive busy) → `False` + log + never raises, `os.open` uses `O_RDONLY|O_NONBLOCK`. `eject` is imported lazily inside a `_eject()` helper so collection still succeeds — the existing 5 `read_drive_status` tests continue to PASS while the 4 new ones fail with `ImportError`.
- `tests/drivers/test_camera_recovery.py` (new) — 5 cases for the not-yet-implemented `capture_frame_with_recovery` sibling of `capture_frame` (`impl-camera-recovery`): first-attempt success skips recovery; STREAMON err → `discover_usb_path()` → unbind → bind → settle sleep → retry succeeds (discovered path appears in argv); non-STREAMON failure does not trigger recovery; both attempts STREAMON-fail → exactly one unbind, two ffmpeg invocations, returns `None`; STREAMON err + `discover_usb_path()` returns `None` → no rebind attempted, returns `None`, warning logged. Uses shared `fake_subprocess` + a local `patched_sleep` fixture. Per `D-camera-autodiscover`: production wires `discover_camera_usb_path` (from `impl-usb-discovery`) as the `discover_usb_path` callable.
- `tests/drivers/test_systemctl.py` (new) — 6 cases for the not-yet-implemented `archivist/drivers/systemctl.py` (`stop_unit` / `start_unit`): argv shape `["sudo", "systemctl", "stop"|"start", <name>]`; returncode 0 → `True`; non-zero → `False` + stderr logged; `FileNotFoundError` (sudo/systemctl missing on a dev box) → `False` with a clear log line. Invariant: never raises.
- `tests/drivers/test_usb_discovery.py` (new) — 4 cases for the not-yet-implemented `discover_camera_usb_path(*, vendor_product="0c45:6366", env_var="ARCHIVIST_CAMERA_USB_PATH")`: sysfs match → bus path returned; two matches → lowest-numbered `videoN` wins; no sysfs match + env var set → env value returned; no sysfs + no env → `None` with a warning that names the expected vendor:product. Uses a `_FakePath` shim + `unittest.mock.patch` on `Path.glob` to keep the suite off real `/sys`.
- Verified: all 4 fail at collection / `ImportError` / `AttributeError` as expected. Existing sprint-1 + sprint-2 Wave 0 tests still PASS. `ruff check tests/drivers/` clean.
- Commits: `10fbd22 test(sprint-2): test-eject`, `0fb679f test(sprint-2): test-camera-recovery`, `2c07059 test(sprint-2): test-systemctl`, and the following `test-usb-discovery` commit.
- Next: drivers Wave 2 (`impl-eject`, `impl-camera-recovery`, `impl-systemctl`, `impl-usb-discovery`). `test-camera-recovery` injects `discover_usb_path` so impl ordering within drivers is flexible — recovery and discovery don't strictly block each other for unit-test green, only for the production wiring in `impl-entrypoint` (pipeline).

### 2026-05-14 — drivers — Wave 0 led-params-refactor landed (D-led-params)

- `archivist/drivers/led.py` — `_get()` now calls `requests.get(f"{base}/cm", params={"cmnd": cmnd}, timeout=5)` instead of building a `?cmnd=...` URL string by hand with `urllib.parse.quote`. Net: one less import (`quote` dropped), encoding is `requests`' responsibility. `_TIMEOUT_SECONDS` unchanged (5.0). The `requests.RequestException` + `ValueError` (JSON decode) catch-and-log-and-return-sentinel behavior is untouched — never-raises invariant still holds.
- `tests/drivers/test_led.py` — URL-introspection assertions reshaped. The four happy-path tests now assert `url == f"{BASE_URL}/cm"` and `kwargs["params"] == {"cmnd": <expected>}` (covering `"Power On"`, `"Power Off"`, `"Power"`). The seven failure-path / never-raises tests are unchanged in shape.
- Verified: `pytest tests/drivers/test_led.py` — 10/10 PASS. `ruff check archivist/drivers/led.py tests/drivers/test_led.py` clean.
- Per `D-led-params` (Decision Log): no new entry needed at task close; the decision was logged at planning time.
